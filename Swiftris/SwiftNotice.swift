...
class SwiftNotice: NSObject {
    ...
    static var center: CGPoint {
        get {
            let screenWith = UIManager.screenWidth()
            let screenHeight = UIManager.screenHeight()
            let x = [0, screenWith / 2, screenWith / 2, 10, screenWith - 10][UIApplication.shared.statusBarOrientation.hashValue] as CGFloat
    ...
    static func noticeOnStatusBar(text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        window.transform = CGAffineTransform(rotationAngle: CGFloat(degress * M_PI / 180))
    ...
    static func wait(imageNames: [UIImage] = [UIImage](), timeInterval: Int = 0) {
        ...
        if imageNames.count > 0 {
            ...
            active.frame = CGRect(x: 21, y: 21, width: 36, height: 36)
            active.startAnimating()
            mainView.addSubview(active)
        }
...
extension UIViewController {
    ...
    func noticeSuccess(text: String, autoClear: Bool = false, autoClearTime: Int = 3) {
        SwiftNotice.showNoticeWithText(type: .success, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    
    func noticeError(text: String, autoClear: Bool = false, autoClearTime: Int = 3) {
        SwiftNotice.showNoticeWithText(type: .error, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    
    func noticeInfo(text: String, autoClear: Bool = false, autoClearTime: Int = 3) {
        SwiftNotice.showNoticeWithText(type: .info, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
